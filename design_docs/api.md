...
- GET `/api/v1/quiz-creation/analysis`
    - **Frontend to BFF**

      ```json
      {
        "description": "アップロード済みソースのツリー構造取得（フロント→BFF）",
        "request": "GET /api/v1/quiz-creation/analysis?sourceId=5001",
        "header": "",
        "body": null,
        "response": {
          "status": 200,
